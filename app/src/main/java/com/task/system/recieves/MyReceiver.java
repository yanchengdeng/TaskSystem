...
import com.task.system.bean.JpushExtraInfo;
...
//                processCustomMessage(context, bundle);
                ...
                JpushExtraInfo jpushExtraInfo = new Gson().fromJson(message,JpushExtraInfo.class);


                if (jpushExtraInfo!=null && !TextUtils.isEmpty(jpushExtraInfo.getTitle())) {

                    if (jpushExtraInfo.getExtras()!=null){
                        Intent goIntent = null;
                        if (jpushExtraInfo.getExtras().getType().equals("1")){
                            goIntent = new Intent(context, TaskDetailActivity.class);
                            Bundle task = new Bundle();
                            task.putString(Constans.PASS_STRING,String.valueOf( jpushExtraInfo.getExtras().getId()));
                            goIntent.putExtras(task);
                        }else if (jpushExtraInfo.getExtras().getType().equals("2")){
                            goIntent = new Intent(context, OpenWebViewActivity.class);
                            Bundle game = new Bundle();
                            game.putString(Constans.PASS_NAME,jpushExtraInfo.getTitle());
                            game.putString(Constans.ARTICAL_TYPE,Constans.INTERGRAY_CODE);
                            game.putString(Constans.PASS_STRING,String.valueOf(jpushExtraInfo.getExtras().getId()));
                            goIntent.putExtras(game);
                        }else if (jpushExtraInfo.getExtras().getType().equals("3")){
